...
fun PengelolaanLaundryScreen(){
    ...
    val db = Room.databaseBuilder(
        context,
        AppDatabase::class.java, "pengelolaan-laundry" // pemanggilan data class dan membuat db "pegelolaan-laundry"
    ).build()
    val setoranLaundryDao = db.setoranLaundryDao()
    val list : LiveData<List<SetoranLaundry>> = setoranLaundryDao.loadAll()
    val items : List<SetoranLaundry> by list.observeAsState(initial = listOf())

    Column(modifier = Modifier.fillMaxWidth()) {
        FormPencatatanLaundry(setoranLaundryDao) // panggil screen FormPencatatanLaundry

        LazyColumn(modifier = Modifier.fillMaxWidth()) { // menggunakan list lazy column
